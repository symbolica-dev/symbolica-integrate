...
fn push_rules_rule_1940(rules: &mut Vec<RubiRule>) {
    rubi_symb!(a__, b__, c__, d__, j_, k_, m_, n_, p_, q_, x_);
    rules.push(rubi_rule!(
        ...
        refs: [],
        pattern: x_.pow(m_) * (a__ * x_.pow(j_) + b__ * x_.pow(k_)).pow(p_) * (c__ + d__ * x_.pow(n_)).pow(q_),
...
fn push_rules_rule_1941(rules: &mut Vec<RubiRule>) {
    rubi_symb!(a__, b__, c__, d__, e__, j_, k_, m_, n_, p_, q_, x_);
    rules.push(rubi_rule!(
        order: 1941,
...
fn push_rules_rule_1942(rules: &mut Vec<RubiRule>) {
    rubi_symb!(a__, b__, c__, d__, e__, j_, jn_, m_, n_, p_, x_);
    rules.push(rubi_rule!(
        order: 1942,
